...
//	private bool bGotFaceRect;
	...
	{
		return isTrackingFace ? thisUserID : 0;
	}
...
//		GC.Collect();
	...
	void Update() 
	{
		if(isFacetrackingInitialized)
		{
			...
			if(kinectManager && kinectManager.IsInitialized())
			{
				thisUserID = kinectManager.GetPrimaryUserID();
			}
			...
			if(sensorData.sensorInterface.UpdateFaceTracking())
			{
				...
				isTrackingFace = sensorData.sensorInterface.IsFaceTracked(thisUserID);
				...
				if(isTrackingFace)
                {
					...
					// get head position
					bGotHeadPos = sensorData.sensorInterface.GetHeadPosition(thisUserID, ref headPos);

					// get head rotation
					bGotHeadRot = sensorData.sensorInterface.GetHeadRotation(thisUserID, ref headRot);

					// get the animation units
					bGotAU = sensorData.sensorInterface.GetAnimUnits(thisUserID, ref dictAU);
                    // print("Get Animation Units " + bGotAU + dictAU[0]);
					// get the shape units
					bGotSU = sensorData.sensorInterface.GetShapeUnits(thisUserID, ref dictSU);
	...
	void OnGUI()
	{
		if(isFacetrackingInitialized)
		{
			if(debugText != null)
			{
				if(isTrackingFace)
				{
					debugText.GetComponent<GUIText>().text = "Tracking - skeletonID: " + thisUserID;
				}
				else
				{
	...
	private void UpdateFaceModelMesh()
	{
		...
		{
			int iNumVertices = sensorData.sensorInterface.GetFaceModelVerticesCount(thisUserID);
			avModelVertices = new Vector3[iNumVertices];
		}

		// get face model vertices
		bGotModelVertices = sensorData.sensorInterface.GetFaceModelVertices(thisUserID, ref avModelVertices);
